...
from dataset_rainset_real import TrainValDataset, TestDataset, TestRealDataset
...
def save_image(derain, name):
    ...
    cv2.imwrite(os.path.join(os.path.join('./result', '{}'.format(name[0]))), derain_save_np)
...
class Session:
    ...
    def get_dataloader(self, dataset_name):
        dataset = TestRealDataset(dataset_name)
...
def run_test(ckp_name):
    ...
    for i, batch in enumerate(dt):
        ...
        save_image(derain, batch['name'])
        psnr_all = psnr_all + psnr
        ssim_all = ssim_all + ssim
        batch_size = batch['O'].size(0)
        all_num += batch_size
        print(f'\rtest---step :{i}/{len(dt)}|ssim :{ssim :.4f} |psnr :{psnr :.4f}', end='', flush=True)

    print('\npsnr_ll:%8f' % (psnr_all / all_num))
    print('ssim_ll:%8f' % (ssim_all / all_num))


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-m', '--model', default='student_20_epoch')
