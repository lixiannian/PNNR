...
class TrainValDataset(Dataset):
    def __init__(self, name):
        ...
        self.root_dir = os.path.join(settings_rainset_real.data_dir, name)
        self.clear_dir = os.path.join(settings_rainset_real.data_dir, 'GT')
        self.real_dir = settings_rainset_real.real_dir
        self.mat_files = os.listdir(self.root_dir)
        self.mat_real = os.listdir(self.real_dir)
        self.patch_size = settings_rainset_real.patch_size
    ...
    def __getitem__(self, idx):
        ...
        if settings_rainset_real.aug_data:
            O, B, R = self.crop(O, B, real_img, aug=True)
            O, B, R = self.flip(O, B, R)
            O, B, R = self.rotate(O, B, R)
        else:
            O, B, R = self.crop(O, B, real_img, aug=False)
    ...
    def crop(self, O, B, real_img, aug):
...
class TestDataset(Dataset):
    def __init__(self, name):
        super().__init__()
        self.rand_state = RandomState(66)
        self.root_dir = os.path.join(settings_rainset_real.test_data_dir, name)
        self.clear_dir = os.path.join(settings_rainset_real.test_data_dir, 'GT')
        self.mat_files = os.listdir(self.root_dir)
        self.patch_size = settings_rainset_real.patch_size
...
class TestRealDataset(Dataset):
    def __init__(self, name):
        super().__init__()
        self.rand_state = RandomState(66)
        self.root_dir = os.path.join(settings_rainset_real.data_dir)
        self.mat_files = os.listdir(self.root_dir)
        self.patch_size = settings_rainset_real.patch_size
        self.file_num = len(self.mat_files)
    ...
    def __getitem__(self, idx):
        ...
        O = np.transpose(O, (2, 0, 1))
        sample = {'O': O, 'B': O, 'name': self.mat_files[idx % self.file_num]}
        return sample
...
class ShowDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if settings_rainset_real.pic_is_pair:
