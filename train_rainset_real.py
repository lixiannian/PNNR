...
import settings_rainset_real as settings
from dataset_rainset_real import TrainValDataset, TestDataset
...
class Session:
    def __init__(self):
        ...
        dict_net = torch.load('./trained_model/rainset/net_latest')
    ...
    def inf_batch(self, name, batch):
        ...
        syn_derain, syn_code = self.net(O)
        real_derain, real_code = self.net(R)
        ...
        if self.ssim_loss == True:
            ...
            kl_streak_loss = 1e-3 * F.kl_div(logp_syn_code, p_real_code, reduction='mean')
            loss = recon_loss + kl_streak_loss
        else:
            loss = l1_loss
        ...
        print(f'\rtrain---step: {self.step} loss : {loss.item():.10f} '
              f'recon_loss : {recon_loss.item():.10f} kl_streak_loss : {kl_streak_loss.item():.10f} | '
              f'ssim: {ssim.item():.5f}', end='', flush=True)
