...
############################################################# 自己写的 patch_embed 和 patch_unembed
def patch_unembed(x, patch_size, x_size):
    # x: (B, H//p*W//p, dim)    x_size: (H, W)
    B, N, dim = x.shape
    x = x.transpose(1, 2).view(B, -1, x_size[0], x_size[1])  # B, C, H, W
    return x


def patch_embed(x, patch_size):
    # x: (B, C, H, W)
    B, C, H, W = x.shape
    x = x.view(B, C * patch_size * patch_size, -1).transpose(1, 2)  # (B, H//p*W//p, C*p*p)
    return x
...
class MRFEB(nn.Module):
    def __init__(self, in_channel, DF1=1, DF2=3, DF3=5):
        super(MRFEB, self).__init__()
...
class RSTB(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        return patch_embed(
            self.conv(
                patch_unembed(
                    self.residual_group(x, x_size), self.patch_size, x_size
                )
            ), self.patch_size
        ) + x
...
class InceptionA(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.conv = nn.Conv2d(in_channels=288, out_channels=in_channels, kernel_size=1)

    def forward(self, source):

        s_branch1x1 = self.branch1x1(source)    # (B, 512, 8, 8) -> (B, 128, 8, 8)

        s_branch3x3 = self.branch3x3(source)    # (B, 512, 8, 8) -> (B, 128, 8, 8)

        s_branch5x5 = self.branch5x5_1(source)
        s_branch5x5 = self.branch5x5_2(s_branch5x5)  # (B, 512, 8, 8) -> (B, 128, 8, 8)

        s_branch7x7 = self.branch7x7_1(source)
        s_branch7x7 = self.branch7x7_2(s_branch7x7)
        s_branch7x7 = self.branch7x7_3(s_branch7x7)  # (B, 512, 8, 8) -> (B, 128, 8, 8)
        ...
        s_branch3x3 = self.avg_pool(s_branch3x3)    # (B, 64, 7, 7) -> (B, 64, 1, 1)
        s_branch5x5 = self.avg_pool(s_branch5x5)
        s_branch7x7 = self.avg_pool(s_branch7x7)

        s_branch1x1 = s_branch1x1.reshape(s_branch1x1.size(0), -1)
        s_branch3x3 = s_branch3x3.reshape(s_branch3x3.size(0), -1)  # (32, 64, 1, 1) -> (32, 64)
        s_branch5x5 = s_branch5x5.reshape(s_branch5x5.size(0), -1)
        s_branch7x7 = s_branch7x7.reshape(s_branch7x7.size(0), -1)

        return source, torch.cat([s_branch1x1, s_branch3x3, s_branch5x5, s_branch7x7], 1)


class SwinIR0(nn.Module):
    r""" SwinIR
    ...
    def __init__(self, img_size=128, patch_size=1, in_chans=3,
        ...
        # self.conv_first = feature_extract(embed_dim=embed_dim)
        ...
        self.inception = InceptionA(in_channels=60)
    ...
    def forward_features(self, x):
        ...
        x = patch_embed(x, self.patch_size)
        if self.ape:
        ...
        x2 = self.layers[1](x1, x_size)
        x3 = self.layers[2](x2, x_size) + x2
        x4 = self.layers[3](x3, x_size) + x1

        x = self.norm(x4)  # B L C
        # x = self.patch_unembed(x, x_size)
        x = patch_unembed(x, self.patch_size, x_size)
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x_mid = self.forward_features(x_first)
            x_mid, loss = self.inception(x_mid)
            res = self.conv_after_body(x_mid) + x_first
...
if __name__ == '__main__':
    ...
    # model = SwinIR0(upscale=1, img_size=(height, width), patch_size=patch_size,
    #                window_size=window_size, img_range=1., depths=[2, 2, 2, 2],
    #                embed_dim=60, num_heads=[6, 6, 12, 12], mlp_ratio=2, upsampler='')
    model = SwinIR0()
    # print(model)
